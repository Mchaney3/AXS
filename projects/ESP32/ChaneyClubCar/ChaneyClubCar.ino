...
#define ledCount 41  // number of LEDs on the strip
int led_BRIGHTNESS = 64;
int led_SPEED = 1000;
int led_MODE = 1;
String led_COLOR = "GREEN";
WS2812FX ws2812fxRoof = WS2812FX(ledCount, roofLedPin, NEO_GRB + NEO_KHZ800);
...
/************************     LED Speed      ************************/
      ...
      {
        Serial.print("Current LED speed: "); Serial.println(ws2812fxRoof.getSpeed());
        ...
        ws2812fxRoof.setSpeed(led_SPEED);
        ws2812fxRoof.service();
...
/************************     LED Brightness      ************************/
      ...
      {
        Serial.print("Current LED Brightness: "); Serial.println(ws2812fxRoof.getBrightness());
        ...
        ws2812fxRoof.setBrightness(led_BRIGHTNESS);
        ws2812fxRoof.service();
        Serial.print("Current LED Brightness: "); Serial.println(ws2812fxRoof.getBrightness());
...
/************************     LED Mode      ************************/
      else if (rxString.indexOf(rx_LED_Mode) != -1) 
      {
        ...
        Serial.print("Current LED mode: "); Serial.println(ws2812fxRoof.getModeName(ws2812fxRoof.getMode()));
        Serial.print("Setting LED Mode: ");  Serial.println(ws2812fxRoof.getModeName(led_MODE));
        ws2812fxRoof.setMode(led_MODE); // segment 0 is leds 0 - 9
        Serial.print("Current LED mode: "); Serial.println(ws2812fxRoof.getModeName(ws2812fxRoof.getMode()));
...
void setup() 
{
  ...
  ws2812fxRoof.setSegment(0,  0,  ledCount-1, FX_MODE_BREATH, BLUE, 1000, false); // segment 0 is led 0 a.k.a. Status LED
  ws2812fxRoof.start();
  ...
  ArduinoOTA
    .onStart([]()
    {
      ...
      ws2812fxRoof.setSegment(0,  0,  1, FX_MODE_BREATH, ORANGE, 1000, false); // segment 0 is led 0 a.k.a. Status LED
      ws2812fxRoof.service();
