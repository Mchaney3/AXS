...
static const int RXPin = 18, // the serial connection to GPS. Note that RX, TX
                 ...
                 //Motor1aPin = 12, Motor1bPin = 13, Motor2aPin = 14, Motor2bPin = 15;

/*  *********** Motor Setup *********** */                 
int8_t motorPins[4] = {12,13,14,15};
...
void setMotors(float left, float right) { 
  ...
  if (left > 0) {
    ...
    analogWrite(motorPins[1], left * 1023);
  } else if (right > 0) {
    analogWrite(motorPins[2], 500);
    analogWrite(motorPins[3], right * 1023);
  } else  {
    analogWrite(motorPins[0], -left * 1023);
    analogWrite(motorPins[1], 500);
    analogWrite(motorPins[2], -right * 1023);
    analogWrite(motorPins[3], 500);
  }
