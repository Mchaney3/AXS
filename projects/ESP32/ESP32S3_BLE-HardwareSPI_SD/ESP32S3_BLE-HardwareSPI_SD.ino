...
// Directory file.
//SdFile root;
// Web server file system object.
//SdFile uploadFile;
//SdFile wsFile;
TaskHandle_t web_server;
#include "Core0.h"    //    Web server initialized on and pinned to core 0 in setup
...
void setup() 
{
  ...
  else
  {
    ...
    Serial.print("SD card initialized with CS pin: "); Serial.println(SS);
    hasSD = true;
  }
  ws2812fx.init();
  ws2812fx.setBrightness(led_BRIGHTNESS);
  ...
  Serial.print("Forward direction set to pin: ");   Serial.println(motorForwardPin);
  Serial.print("Backward direction set to pin: ");   Serial.println(motorBackwardPin);
  ...
  ArduinoOTA.begin();
  Serial.print("OTA initialized at "); Serial.println(WiFi.localIP());
  ...
  // Start advertising
  pServer->getAdvertising()->start();
  Serial.print("BLE initialized as "); Serial.println(deviceName);
  ...
  xTaskCreatePinnedToCore(
                    ...
                    0,            /* Match Core 1 priority */
                    &web_server,  /* Task handle to keep track of created task */
                    0);           /* pin Web Server to core 0 */                  
  delay(500);
